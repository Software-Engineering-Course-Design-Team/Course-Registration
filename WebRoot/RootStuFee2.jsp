...
<%@ page language="java" contentType="text/html; charset=UTF-8" 
		 pageEncoding="UTF-8"%>
<%@ page import="java.util.*,java.io.*,com.model.javabean.Student,com.model.javabean.Stufee,java.util.ArrayList.*"%>
<html lang="en">
<head>
    <meta charset="UTF-8">
   ...
   <!-- 顶部开始 -->
	...
	<div class="left-nav">
        ...
        <div class="page-content">
          <div class="content">
            ...
            <form class="layui-form" action="AdminStuFee" method="post">
    ...
    border-radius:10px;" value="查看未缴费名单">
            ...
            <table class="layui-table">
                ...
                <tbody>
                <%
                	ArrayList<Student> stuinfo=(ArrayList<Student>)request.getAttribute("student");
                    ArrayList<Stufee> info=(ArrayList<Stufee>)request.getAttribute("NoFeeInfo");
                    ArrayList<String> depinfo=(ArrayList<String>)request.getAttribute("depinfo");
                    Iterator e1 = stuinfo.iterator();	
					Iterator e = info.iterator();
					Iterator e2 = depinfo.iterator();
					while(e.hasNext()){
						Stufee a=(Stufee)e.next();
						Student b=(Student)e1.next();
                %>
                <form action="AdminUpdDelFee" method="post">
                <tr>
                    <th>
                           <%=a.getSID() %>
                        </th>
                       ...
                       <%=a.getTerm() %>
                        ...
                        <th>
                        <input type="hidden" name="userid" value=<%=a.getSID()%>>
                        <input type="hidden" name="term" value=<%=a.getTerm()%>>
                        <input type="hidden" name="info" value=<%=request.getParameter("info") %>>
                        <input type="hidden" name="stuid" value=<%=request.getParameter("stuid") %>>
                       	<%if(a.getStatus().equals("未缴清")){%>
                       	<input type="submit" name="op" style="background: transparent;border:none;
    outline:none;font-size: 13px;color:#fff;background: #9A6159;padding:8px 11px;cursor: pointer;
    border-radius:10px;" value="缴费" onclick="confirmUpdate()">
                     <% }%>
    					<input type="submit" name="op" style="background: transparent;border:none;
    outline:none;font-size: 13px;color:#fff;background: #9A6159;padding:8px 11px;cursor: pointer;
